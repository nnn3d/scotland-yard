...
// @ts-ignore
export function createUseSelector(...args: any[]) {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  const selector = createArraySelector(...args)
  const selectorUse = selector as typeof selector & {
    use: () => ReturnType<typeof selector>
    get: () => ReturnType<typeof selector>
  }
  selectorUse.get = () => {
    const selectors = args.slice(0, -1)
    const resultFunc = args[args.length - 1]

    const resultArgs = selectors.map((selector) => {
      if (selector.resultFunc && selector.recomputations) {
        return selector.get()
      }
      return selector()
    })

    return resultFunc(...resultArgs)
  }
  selectorUse.use = function useData(...args: any[]) {
    return useSelector(
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      args.length ? (state) => selector(state, args[0]) : selector,
    )
  }
  return selectorUse
}
...
export type CreateUseSelector = typeof createUseSelector
